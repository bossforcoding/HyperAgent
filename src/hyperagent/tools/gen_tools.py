...
class EditorTool(BaseTool):
    ...
    def _run(self, relative_file_path: str = None, start_line: int = None, end_line: int = None, patch: str = None,
        ...
        if self.language == "python":
            ...
            command_fix = f"autopep8 --in-place --aggressive {patch_file_path}"
            result = subprocess.run(command_fix, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
...
class OpenFileToolForGeneratorArgs(BaseModel):
    ...
    keywords: List[str] = Field(...,
                                description="The list of keywords you want to search in the file, it's should be a single word like grpc but not grpcio.grpc")
...
class OpenFileToolForGenerator(BaseTool):
    ...
    def _run(self, relative_file_path: str, start_line: Optional[int] = None, end_line: Optional[int] = None,
             keywords: List[str] = [], preview_size: int = 10, max_num_result: int = 5):
        """
        Opens the specified file and returns its content.
                    ...
                    if len(line_idx) == 0:
                        ...
                        for i in range(len(line_idx)):
                            if line_ranges[i] is None:
                                ...
                                expanded_source = "\n".join(lines[max(0, line_idx[i] - preview_size):min(len(lines),
                                                                                                         line_idx[
                                                                                                             i] + preview_size)])
                                ...
                                if checkcover(line_ranges[i], line_ranges_overall):
                                    ...
                                    expanded_source = "\n".join(
                                        lines[max(0, line_ranges[i][0]):min(len(lines), line_ranges[i][1])])
...
if __name__ == "__main__":
    open_file_gen = OpenFileToolForGenerator(path='/datadrive5/huypn16/HyperAgent-Master/data/repos/Chart-7',
                                             language="java")
    result = open_file_gen._run(relative_file_path="src/main/java/org/jfree/data/time/TimePeriodValues.java",
                                keywords=["class TimePeriodValues", "updateBounds"])
    print(result)
